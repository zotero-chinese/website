...
export function useLanguageNotification() {
  ...
  const targetUrl = computed(() => {
    ...
    if (isCurrentChinese.value) return `/en${path}`
    else return `${path.replace(/^\/en\//, '/')}`
  })

  onMounted(() => {
    if (!inBrowser || hasShownNotification.value) return

    if (isBrowserChinese.value === isCurrentChinese.value) return
    ...
    ElMessageBox.confirm(
      ...
      isBrowserChinese.value && !isCurrentChinese.value ? '切换语言' : 'Switch Language',
      {
